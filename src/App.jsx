    ...
    <div className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2 lg:grid-cols-4">
      <label className="text-sm text-slate-700">
        ...
        <div className="mt-1 flex items-center gap-2">
          <button onClick={() => setSpeed((prev) => Math.max(12, prev - 2))} className="rounded-md border border-slate-300 p-1">
            <Rewind size={14} />
          </button>
          <input type="range" min="12" max="56" value={speed} onChange={(event) => setSpeed(Number(event.target.value))} className="w-full" />
          <button onClick={() => setSpeed((prev) => Math.min(56, prev + 2))} className="rounded-md border border-slate-300 p-1">
            <FastForward size={14} />
          </button>
        </div>
      </label>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:col-span-2 lg:col-span-4">
        ...
        <button onClick={resetPosition} className="flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100">
        ...
        <button onClick={stop} className="rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100">Stop / Exit</button>
      </div>
        ...
        <div ref={stageRef} className="space-y-3">
          ...
          <div className="rounded-xl bg-slate-950 p-2 sm:p-3">
            ...
            >
              ...
              <div
                ref={contentRef}
                className={`absolute left-1/2 w-[88%] max-w-5xl -translate-x-1/2 ${activePreset.align} ${activePreset.textColor} ${fontClasses[fontFamily]} ${
                  activePreset.uppercase ? 'uppercase tracking-wide' : 'normal-case tracking-normal'
                }`}
                style={{
                  transform: `translateY(${crawlY}px) rotateX(${activePreset.tilt}deg)`,
