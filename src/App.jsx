  ...
  if (running) {
    return (
      <DrillCard title="Drill E: Star-Wars Style Teleprompter" icon={Play}>
        <div className="mb-3 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-3">
          <label className="text-sm text-slate-700 sm:col-span-1">
            Speed ({speed}px/s)
            <input
              type="range"
              min="12"
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100"
          >
        ...
        <div className="rounded-xl bg-slate-950 p-3">
          <div
            ref={viewportRef}
            className={`relative overflow-hidden rounded-lg bg-[radial-gradient(circle_at_top,_#1e293b_0%,_#020617_65%)] ${
              isFullscreen ? 'h-screen w-screen rounded-none' : 'h-80 sm:h-[26rem]'
            }`}
          ...
          >
            ...
            <div
              ref={contentRef}
              className="absolute left-1/2 w-[78%] max-w-4xl text-center font-semibold uppercase tracking-wide text-amber-200 md:w-[70%]"
              style={{
                transform: `translate(-50%, ${crawlY}px) rotateX(24deg)`,
                transformOrigin: '50% 100%',
                fontSize: `clamp(18px, ${textSize / 18}vw, ${textSize}px)`,
              }}
            >
              {lines.map((line, idx) => (
