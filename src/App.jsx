...
Speak every line with clarity and power.`,
  ...
  const applyCrawlTransform = () => {
    if (!contentRef.current) return;
    contentRef.current.style.transform = `translate(-50%, ${crawlYRef.current}px) rotateX(${activePreset.tilt}deg)`;
  };
  ...
  useEffect(() => {
    const onFullscreenChange = () => {
      ...
      setShowFullscreenControls(!active);
    };
  ...
  useEffect(() => {
    ...
    crawlYRef.current = viewportRef.current.clientHeight + 100;
    applyCrawlTransform();
  ...
  }, [running, speed, paused, styleMode, onDrillComplete]);

  useEffect(() => {
    applyCrawlTransform();
  }, [textSize, styleMode, fontFamily, lineSpacing]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (event.key.toLowerCase() === 'c' && isFullscreen) {
  ...
  const TeleprompterControls = ({ floating = false }) => (
    <div
      className={`grid gap-3 rounded-xl border p-3 sm:grid-cols-2 lg:grid-cols-4 ${
        floating ? 'border-slate-700 bg-slate-900/90 text-slate-100 backdrop-blur' : 'border-slate-200 bg-slate-50'
      }`}
    >
      <label className={`text-sm ${floating ? 'text-slate-100' : 'text-slate-700'}`}>
        Speed ({speed}px/s)
      ...
      <label className={`text-sm sm:col-span-2 lg:col-span-4 ${floating ? 'text-slate-100' : 'text-slate-700'}`}>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:col-span-2 lg:col-span-4">
        <button
          ...
          className="flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100 hover:text-slate-900"
        >
          {paused ? <Play size={15} /> : <Pause size={15} />}
          {paused ? 'Resume' : 'Pause'}
        </button>
        <button type="button" onClick={resetPosition} className="flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100 hover:text-slate-900">
        ...
        <button type="button" onClick={stop} className="rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100 hover:text-slate-900">Stop / Exit</button>
      </div>
      <p className={`text-xs sm:col-span-2 lg:col-span-4 ${floating ? 'text-slate-300' : 'text-slate-500'}`}>
        Keyboard: Space play/pause · ↑/↓ speed · ←/→ text size · R restart · F fullscreen · C reveal controls in fullscreen
      </p>
    </div>
  );
  ...
  if (running) {
    return (
      <DrillCard title="Drill E: Star-Wars Style Teleprompter" icon={Play}>
        <div ref={stageRef} className="relative space-y-3">
          {!isFullscreen && <TeleprompterControls />}
          ...
          <div className="rounded-xl bg-slate-950 p-2 sm:p-3">
            ...
            >
              ...
              <div
                ...
                style={{
                  transform: `translate(-50%, ${crawlYRef.current}px) rotateX(${activePreset.tilt}deg)`,
            ...
            <>
              {!showFullscreenControls && (
                <button
                  ...
                  className="fixed bottom-5 right-5 z-50 rounded-full bg-slate-900/80 px-4 py-2 text-xs font-medium text-white backdrop-blur hover:bg-slate-800"
                >
                  Reveal Controls
                </button>
              )}
                ...
                <div className="fixed inset-x-2 top-2 z-50 space-y-2 sm:inset-x-4">
                  <div className="flex justify-end">
                    <button
                      ...
                      className="rounded-full bg-slate-900/90 px-3 py-1 text-xs text-white"
                    >
                      Hide Controls
                    </button>
                  </div>
                  <TeleprompterControls floating />
                </div>
              )}
            </>
          )}
